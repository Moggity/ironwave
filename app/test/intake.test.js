...
test('a 10 minute powerlifting session cap is refused', () => {
  const issues = Engine.validateIntake({ timeMode: 'custom', timeCapMin: 10, meetChoice: 'none' }, PL, NOW);
...
test('a 45 minute powerlifting cap and a 30 minute bodybuilding cap pass', () => {
  assert.deepStrictEqual(
    Engine.validateIntake({ timeMode: 'custom', timeCapMin: 45, meetChoice: 'none' }, PL, NOW), []);
  assert.deepStrictEqual(
    Engine.validateIntake({ timeMode: 'custom', timeCapMin: 30 }, BB, NOW), []);
});

test('a 20 minute bodybuilding cap is refused (30 minute floor)', () => {
  assert.deepStrictEqual(
    keysOf(Engine.validateIntake({ timeMode: 'custom', timeCapMin: 20 }, BB, NOW)),
    ['val.time_min']);
});

test('custom time mode with no minutes entered is an error, not unlimited', () => {
  assert.deepStrictEqual(
    keysOf(Engine.validateIntake({ timeMode: 'custom', timeCapMin: '', meetChoice: 'none' }, PL, NOW)),
    ['val.time_required']);
});

test('unlimited time mode never raises a time issue', () => {
  assert.deepStrictEqual(
    Engine.validateIntake({ timeMode: 'unlimited', meetChoice: 'none' }, PL, NOW), []);
});

test('a meet date tomorrow is refused with the runway reason', () => {
  const issues = Engine.validateIntake(
    { timeMode: 'unlimited', meetChoice: 'date', meetDate: iso(NOW + DAY) }, PL, NOW);
  assert.deepStrictEqual(keysOf(issues), ['val.meet_too_soon']);
  assert.strictEqual(issues[0].params.min, 28);
});

test('a meet date over a year out is refused', () => {
  assert.deepStrictEqual(
    keysOf(Engine.validateIntake(
      { timeMode: 'unlimited', meetChoice: 'date', meetDate: iso(NOW + 500 * DAY) }, PL, NOW)),
    ['val.meet_too_far']);
});
...
test('a meet 10 weeks out passes clean', () => {
  assert.deepStrictEqual(
    Engine.validateIntake(
      { timeMode: 'unlimited', meetChoice: 'date', meetDate: iso(NOW + 70 * DAY) }, PL, NOW), []);
});

test('an unanswered meet question blocks; choosing a date without one blocks', () => {
  assert.deepStrictEqual(
    keysOf(Engine.validateIntake({ timeMode: 'unlimited' }, PL, NOW)),
    ['val.meet_choice']);
  assert.deepStrictEqual(
    keysOf(Engine.validateIntake({ timeMode: 'unlimited', meetChoice: 'date' }, PL, NOW)),
    ['val.meet_date_required']);
});

test('bodybuilding never raises meet issues, even with an absurd date on the draft', () => {
  assert.deepStrictEqual(
    Engine.validateIntake({ timeMode: 'unlimited', meetDate: iso(NOW + DAY) }, BB, NOW), []);
});
...
test('the meet step refuses to advance on a too-soon date', () => {
  const step = draftAt('meet', { track: 'powerlifting', daysPerWeek: 4, daysMode: 'count',
...
test('the meet step advances on a valid date and on an explicit no-meet', () => {
  let step = draftAt('meet', { track: 'powerlifting', daysPerWeek: 4, daysMode: 'count',
    meetChoice: 'date', meetDate: iso(Date.now() + 70 * DAY) });
  app.obNext(step);
  assert.strictEqual(app.V.obStep, step + 1, 'valid date advances');
  step = draftAt('meet', { track: 'powerlifting', daysPerWeek: 4, daysMode: 'count',
    meetChoice: 'none' });
  app.obNext(step);
...
test('the time step refuses a below-floor cap and accepts one at the floor', () => {
  let step = draftAt('time', { track: 'powerbuilding', daysPerWeek: 4, daysMode: 'count',
...
test('a stale Continue for another step is a no-op', () => {
  const step = draftAt('experience', { track: 'powerlifting', daysPerWeek: 4, daysMode: 'count',
...
test('a powerlifting flow with a valid meet date builds the jm2-peak taper', () => {
  ...
  const p = runFlow({ track: 'powerlifting', daysPerWeek: 4, daysMode: 'count',
...
test('a bodybuilding flow never sees the meet step and still asks focus', () => {
  const p = runFlow({ track: 'bodybuilding', goalArchetype: 'recomp',

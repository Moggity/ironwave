...
## 9. Tier boundary at launch (2026-07-17, owner rulings)

Two rulings correct this document's first draft:

**Ruling A: nothing is enforced in the prototype.** The debug toggle is a
design-preview tool. Beta testers switching themselves to Coach is fine
and useful. Every constraint below is a LAUNCH-build requirement attached
to the slice that ships it — none is prototype work, and a briefly-added
free-mode gate on the week preview was reverted for exactly this reason.

**Ruling B: on lapse, the logger detaches from the coach completely.**
No degraded-program machinery. When a subscription ends: a card announces
it; the next week arrives as a clone of last week's exercises (structure
only, no prescriptions) so nobody is stranded mid-routine; every week
after that is empty and the athlete logs by hand, Strong-style (Epic L's
freestyle logging). History and export untouched; re-subscribing
re-engages the coach from stored state. This supersedes the earlier
"your program, as routines" design in L4 and the monetization report's
original M2 mechanism.

Ruling B makes most enforcement worry moot: a detached calendar has no
future coach weeks to browse, and the adaptation loop (the thing the
tier actually sells) ends with the subscription by construction. What
remains are ordinary launch-build constraints, logged as **TB items** in
`docs/pending-future-work.md`, each riding an existing slice:

- **TB3** (with M4/H7): generation gates live on the functions
  (`makeProgram`, `programFromTemplate`, imports), not on buttons —
  circulated template JSONs land as structure, never live programs.
- **TB4** (with R1/M3): the entitlement cache lives in device-scoped
  storage, never inside `S`; production import ignores entitlement-shaped
  fields (including `debugTier`). Export/import stays free and sacred.
- **TB5** (with M1/R3): store builds ignore `S.debugTier`; the Settings
  toggle hides behind a dev flag. (Prototype and beta keep it visible.)
- **TB6** (with L2): coach-derived picker hints (SFR ordering, adds-head,
  over-MRV, time-cost) element-gate away in free routine editing.
- **TB7** (with T1/T3): `vReport` and every receipt surface check the
  seam from birth.
- **TB8** (with M3): offline grace keys off a device-stored last-verified
  timestamp; clock rollback reads as expired-pending-check.

Dropped: the old TB1 (subsumed by Ruling B — detachment IS the brain
pausing) and TB2 (trial preview depth; not a problem worth solving —
screenshots of an adaptive plan go stale on the first AMRAP). Store-level
trial farming is the stores' own enforcement and stays out of scope.

**The one-sentence version:** free is a perfect memory and coach is
judgment; make every act of judgment visible as a receipt so the trial
can prove what $79.99 buys, keep the boundary behind one `hasCoach()`
seam, and at launch let a lapse simply detach the calendar — a card, one
cloned week, then empty weeks — because a detached logger needs no
policing and the adaptation loop ends with the subscription by
construction.
